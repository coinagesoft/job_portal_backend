...
namespace JobPortal.Services.IImplement.IAdmin
{
    ...
    {
        Task<CommonResponseDto> CreatePlanAsync(
            CreateCreditPlanRequestDto request,
            Guid adminId);

        Task<CommonResponseDto> UpdatePlanAsync(
            UpdateCreditPlanRequestDto request,
            Guid adminId);

        Task<CommonResponseDto> DeletePlanAsync(
            Guid planId,
            Guid adminId);

        Task<List<CreditPlanResponseDto>>
            GetAllPlansAsync(Guid adminId);

        Task<CreditPlanResponseDto?>
            GetPlanByIdAsync(
                Guid planId,
                Guid adminId);
    }
